...
func determineTarget(initialData []byte, remoteAddr net.Addr) string {
	...
	if strings.Contains(string(initialData), "subs") {
		log.Println("A new connection from", remoteAddr, "and requested subscribe so we'll forward them to", apacheTarget)
		return apacheTarget
	}
